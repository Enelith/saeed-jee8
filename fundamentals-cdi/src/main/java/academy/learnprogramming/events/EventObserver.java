...
public class EventObserver implements Serializable {
	...
	logger.log(Level.INFO,
		    "User {0} logged in at {1}. Logged from plain event observer",
		    new Object[] {
	...
	// We will just log it
	logger.log(Level.INFO,
		    "User {0} logged in at {1}. Logged from qualified observer",
		    new Object[] {
			    eventData.getEmail(), eventData.getLoginTime() });
	// try {
	// Thread.sleep(6000);
	...
	// We will just log it
	logger.log(Level.INFO,
		    "User {0} logged in at {1}. Logged from async observer",
		    new Object[] {
			    eventData.getEmail(), eventData.getLoginTime() });
	try {
	    Thread.sleep(6000);
