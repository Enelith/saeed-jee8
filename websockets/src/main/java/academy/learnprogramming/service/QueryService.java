...
public class QueryService {
	...
	return entityManager.createNamedQuery(Employee.EMPLOYEE_CONSTRUCTOR_PROJ, EmployeeDetails.class)
		    .getResultList();
    }
	...
	TypedQuery<Employee> filter =
		    entityManager.createQuery("select e from Employee e where e.fullName LIKE :filter", Employee.class)
	...
	return entityManager
		    .createQuery("select e from Employee e where e.address.state in ('NY', 'CA')", Employee.class)
		    .getResultList();
    }
	...
	return entityManager.createQuery("select e from Employee e where e.fullName LIKE :filter", Employee.class)
		    ...
		    .getResultList();// jo% jonathan, joseph, joe
    }

    // public Collection<Employee> getEmployeesByBonus() {
    // return entityManager.createQuery("select e, e.basicSalary * 0.15 as bonus
    // from Employee e order by bonus ", Employee.class).getResultList();
    // }
	...
	return entityManager.createQuery(
		    "select d.departmentName, avg(e.basicSalary) from Department d join d.employees e where e.subordinates is empty  group by d.departmentName having avg(e.basicSalary) > :minThreshold",
		    Object[].class)
	...
	ApplicationUser user =
		    entityManager.createNamedQuery(ApplicationUser.FIND_USER_BY_CREDENTIALS, ApplicationUser.class)
